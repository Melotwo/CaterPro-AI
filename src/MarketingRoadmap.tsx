...
const MarketingRoadmap: React.FC = () => {
  const [missions, setMissions] = useState([
    { id: 1, title: 'Share 100% Grade on LinkedIn', category: 'Social', completed: false, icon: '🏆', action: 'Share Success' },
    { id: 2, title: 'Convert Direct Traffic to Referrals', category: 'Growth', completed: false, icon: '🔗', action: 'Pitch Schools' },
    { id: 3, title: 'Check Google Analytics Realtime', category: 'Data', completed: true, icon: '📈', action: 'View Dashboard' },
    { id: 4, title: 'Analyze US vs SA User Intent', category: 'Search', completed: false, icon: '🌐', action: 'View Stats' },
  ]);
  ...
  return (
    <div className="mt-12 animate-slide-in">
      <div className="bg-slate-900 rounded-3xl p-8 text-white shadow-2xl relative overflow-hidden">
        <div className="absolute top-0 right-0 p-12 opacity-5 pointer-events-none text-[160px]">
            🎯
        </div>
        ...
        <div className="relative z-10">
                ...
                {missions.map((mission) => (
                    <div key={mission.id} className="bg-slate-800/50 border border-slate-700 p-5 rounded-2xl flex items-center justify-between group hover:border-primary-500/50 transition-all">
                        <div className="flex items-center gap-4">
                            <div className={`p-3 rounded-xl ${mission.completed ? 'bg-green-500/20 text-green-500' : 'bg-slate-700 text-slate-400'} text-xl`}>
                                {mission.icon}
                            </div>
                            <div>
                        ...
                        {mission.id === 1 ? (
                            ...
                            <span className="text-slate-600 group-hover:text-primary-500 transition-colors text-xl">➡️</span>
                        )}
                    </div>
                ))}
            </div>
            ...
            <div className="mt-8 pt-8 border-t border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-4">
                <div className="flex items-center gap-2 text-xs font-bold text-slate-400">
                    🌐 Global SEO Rating: <span className="text-green-500">High</span>
                </div>
                <div className="flex items-center gap-4">
