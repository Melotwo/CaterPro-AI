  ...
  return (
    <div className="relative" ref={wrapperRef}>
      <div className="w-full">
        <label htmlFor="multiselect-button" className="block text-sm font-medium text-slate-700 dark:text-slate-300">
        ...
        <button
          ...
          className="mt-1 flex items-center justify-between w-full min-h-[42px] px-3 py-2 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-md shadow-sm text-left focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 dark:focus:ring-offset-slate-950 transition-colors"
        ...
        <div className="absolute z-10 mt-1 w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-md shadow-lg max-h-60 flex flex-col">
          <div className="p-2 sticky top-0 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
             <div className="relative">
                ...
                />
                <span className="pointer-events-none absolute top-1/2 -translate-y-1/2 right-3 h-4 w-4 text-slate-400">🔍</span>
             </div>
          </div>
          <div className="overflow-y-auto">
            {filteredOptions.length > 0 ? (
                ...
                <p className="text-center py-4 text-sm text-slate-500">No options found.</p>
            )}
          </div>
        </div>
      )}
