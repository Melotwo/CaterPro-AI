    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
        {MENU_SECTIONS.map(({ title, key }, catIdx) => {
          ...
          const isWideSection = key === 'shoppingList' || key === 'recommendedEquipment';
          ...
          if (key === 'shoppingList') {
            ...
            return (
               <div key={key} className={`border-2 border-slate-100 dark:border-slate-800 rounded-xl ${sectionClass}`}>
                                                  ...
                                                  <div key={checkKey} className="flex items-center gap-2">
                                                      ...
                                                      >
                                                          ...
                                                          <div className="flex-1">
                                                              ...
                                                              {/* Enhanced budget and sourcing info */}
                                                              {(item.brandSuggestion || item.estimatedCost) && !isBulkEditMode && (
                                                                  <div className={`mt-1.5 pl-1 text-xs space-y-1 transition-colors ${isChecked ? 'text-slate-400/80 dark:text-slate-500/80' : 'text-slate-500 dark:text-slate-400'}`}>
                                                                      {item.estimatedCost && (
                                                                          <p><strong className="font-semibold text-slate-600 dark:text-slate-300">Est. Cost:</strong> {item.estimatedCost}</p>
                                                                      )}
                                                                      {item.brandSuggestion && (
                                                                          <p><strong className="font-semibold text-slate-600 dark:text-slate-300">Brands:</strong> {item.brandSuggestion}</p>
                                                                      )}
                                                                  </div>
                                                              )}
                                                              {/* Affiliate marketing enhancement: display item description if available */}
                                                              {item.description && !isBulkEditMode && (
                                                                  <p className={`text-xs mt-1.5 italic pl-1 transition-colors ${isChecked ? 'text-slate-400/80 dark:text-slate-500/80' : 'text-slate-500 dark:text-slate-400'}`}>
                                                                  {item.description}
                                                                  </p>
                                                              )}
                                                              {isEditable ? (
          ...
          if (key === 'recommendedEquipment') {
            ...
            return (
              <div key={key} className={`border-2 border-slate-100 dark:border-slate-800 p-4 sm:p-6 rounded-xl ${sectionClass}`}>
