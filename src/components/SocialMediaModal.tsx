...
import { X, Copy, Image as ImageIcon, Check, RefreshCw, Smartphone, Linkedin, Twitter, Facebook, MessageCircle, Send, Film, Play, Download, Zap, Rocket, ExternalLink } from 'lucide-react';
                    ...
                    {activeMode === 'sell' ? (
                        <div className="max-w-sm w-full space-y-4">
                            <div className="bg-indigo-600 text-white p-8 rounded-2xl shadow-xl flex flex-col items-center text-center">
                                ...
                                <p className="text-indigo-100 text-sm">Post your founder story on LinkedIn. Use the 'Sell' mode caption to announce your lifetime deal.</p>
                            </div>
                            <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-indigo-100 dark:border-indigo-900 shadow-sm">
                                <h4 className="text-xs font-bold uppercase text-slate-400 mb-2">Pro Tip for LinkedIn</h4>
                                <p className="text-xs text-slate-600 dark:text-slate-400 mb-3">LinkedIn caches your website image. If the thumbnail looks old, use the Post Inspector to clear it.</p>
                                <a 
                                ...
                                >
                                    <ExternalLink size={12} /> Open LinkedIn Post Inspector
                                </a>
                            </div>
                        </div>
                    ) : activeMode === 'create' ? (
                ...
                <div className="md:w-1/2 flex flex-col h-full bg-white dark:bg-slate-900">
                    ...
                    <div className="p-4 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50 flex flex-col gap-3">
                        <button onClick={() => handleCopyText(editedCaption, setCopiedText)} className="w-full py-3 bg-slate-800 hover:bg-slate-900 text-white rounded-lg font-bold flex items-center justify-center gap-2">
                            {copiedText ? <Check size={18} /> : <Copy size={18} />} {copiedText ? 'Copied!' : 'Copy Text'}
                        </button>
                        <button onClick={openPlatform} className="w-full py-3 rounded-lg font-bold border-2 border-primary-500 text-primary-600 hover:bg-primary-500 hover:text-white transition-colors capitalize">
                            Open {activePlatform} & Post
                        </button>
                    </div>
                </div>
            </>
            ...
            <div className="w-full h-full flex flex-col p-6 bg-slate-50 dark:bg-slate-900/50 overflow-y-auto">
                <div className="max-w-3xl mx-auto w-full flex flex-col h-full gap-6">
                    <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 flex-1 flex flex-col">
                        <h3 className="font-bold text-slate-800 dark:text-white mb-2 flex items-center gap-2"><MessageCircle className="text-amber-500" /> Paste User Comment</h3>
                        <textarea className="flex-grow w-full p-4 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg resize-none focus:ring-2 focus:ring-amber-500 focus:outline-none dark:text-white" placeholder='e.g., "Do you do Halal menus?"' value={incomingComment} onChange={(e) => setIncomingComment(e.target.value)} />
                        <div className="mt-4 flex justify-end">
                            <button onClick={handleGenerateReply} disabled={!incomingComment.trim() || isGeneratingReply} className="bg-amber-500 hover:bg-amber-600 text-white px-6 py-2 rounded-lg font-bold flex items-center gap-2 disabled:opacity-50">
                                {isGeneratingReply ? <RefreshCw className="animate-spin" size={18} /> : <Send size={18} />} Generate Reply
                            </button>
                        </div>
                    </div>
                    {generatedReply && (
                         <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-lg border-2 border-amber-100 dark:border-amber-900/30 animate-scale-up">
                            <h3 className="font-bold text-slate-800 dark:text-white mb-2 flex items-center gap-2"><Check className="text-green-500" /> AI Suggested Reply</h3>
                            <div className="p-4 bg-slate-50 dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-700 mb-4 text-slate-700 dark:text-slate-300 text-sm">{generatedReply}</div>
                            <button onClick={() => handleCopyText(generatedReply, setCopiedText)} className="w-full py-3 bg-slate-800 hover:bg-slate-900 text-white rounded-lg font-bold flex items-center justify-center gap-2">
                                {copiedText ? <Check size={18} /> : <Copy size={18} />} {copiedText ? 'Copied!' : 'Copy Reply'}
                            </button>
                         </div>
                    )}
                </div>
