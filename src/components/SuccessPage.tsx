...
export const SuccessPage: React.FC<SuccessPageProps> = ({ 
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 py-24 noise-bg">
      <QuickInfoModal 
        ...
        <div className="p-16">
          <div className="grid md:grid-cols-2 gap-10 mb-16">
            <button 
              ...
              className="bg-white/60 backdrop-blur-sm p-10 rounded-[3rem] border border-white/80 shadow-sm relative overflow-hidden group hover:border-emerald-500/30 transition-all text-left active:scale-95"
            >
              <div className="absolute top-0 right-0 w-24 h-24 bg-emerald-500/5 mask-triangle -z-10" />
              <span className="text-[10px] font-black text-slate-700 uppercase tracking-[0.4em] block mb-6">Financial Summary</span>
              <div className="flex items-baseline gap-3">
                <span className="text-5xl font-anchor text-charcoal">{formatCurrency(proposal?.costPerHead * 50)}</span>
                <span className="text-slate-500 font-bold text-sm uppercase tracking-widest">Total Est.</span>
              </div>
              <p className="text-slate-700 text-xs mt-4 font-medium italic">Based on 50 guests @ {formatCurrency(proposal?.costPerHead)}/head</p>
            </button>
            ...
            >
              <div className="absolute top-0 right-0 w-24 h-24 bg-emerald-500/5 mask-triangle -z-10" />
              <span className="text-[10px] font-black text-slate-700 uppercase tracking-[0.4em] block mb-6">Yield Intelligence</span>
              <div className="flex items-baseline gap-3">
                <span className="text-5xl font-anchor text-emerald-600">{100 - (proposal?.wasteYieldAnalysis?.yieldPercentage || 100)}%</span>
                <span className="text-slate-500 font-bold text-sm uppercase tracking-widest">Waste</span>
              </div>
              <p className="text-slate-700 text-xs mt-4 font-medium italic">Efficiency Rating: {proposal?.wasteYieldAnalysis?.yieldPercentage || 100}% Yield</p>
            </button>
          </div>
