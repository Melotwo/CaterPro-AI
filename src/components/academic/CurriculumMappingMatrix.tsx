            ...
            return (
              <button
                ...
                className={`p-4 rounded-2xl border text-left transition-all relative overflow-hidden cursor-pointer ${
                  isSelected
                    ? 'bg-gradient-to-br from-red-950/60 via-slate-900 to-red-900/40 border-red-600 shadow-lg shadow-red-600/20 ring-1 ring-red-500/40'
                    : 'bg-slate-950/60 border-white/5 hover:border-white/20 hover:bg-slate-800/50 text-slate-400'
                }`}
              >
                <div className="flex items-start justify-between gap-2 mb-2">
                  <span className="text-[10px] font-mono font-bold px-2 py-0.5 rounded bg-red-950/40 text-red-400 border border-red-800/60">
                    SAQA ID {qual.saqaId}
                  </span>
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-white/10 pb-5">
          <div>
            <div className="flex flex-wrap items-center gap-2 mb-1.5">
              <span className="px-2.5 py-0.5 rounded bg-red-950/40 text-red-400 border border-red-800/60 text-[10px] font-black uppercase tracking-wider">
          ...
          <div className="lg:text-right shrink-0 space-y-1 bg-slate-950/50 p-3.5 rounded-2xl border border-white/5">
            ...
            <div className="flex flex-wrap lg:justify-end gap-1.5 max-w-xs">
              {activeQual.targetRoles.map((role, idx) => (
                <span key={idx} className="text-[10px] font-bold px-2 py-0.5 rounded bg-slate-800 text-red-200 border border-red-800/40">
                  {role}
                </span>
              ))}
            </div>
            ...
            return (
              <div
                ...
                className={`p-5 rounded-2xl border transition-all cursor-pointer text-left ${
                  isModSelected
                    ? 'bg-red-950/30 border-red-600/60 ring-1 ring-red-500/40 shadow-lg'
                    : 'bg-slate-950/60 border-white/10 hover:border-red-600/40 hover:bg-slate-900/60'
                }`}
              >
                ...
                <div className="space-y-2 mb-3">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {mod.caterproToolAlignment.map((tool, tIdx) => (
                      <span 
                        key={tIdx} 
                        className="text-[10px] font-bold px-2 py-0.5 rounded-md bg-red-950/40 text-red-400 border border-red-800/60 flex items-center gap-1"
                      >
                    ...
                    >
                      ...
                      {onOpenLessonPlanForModule && (
                        <button
                          ...
                          className="w-full py-2 px-3 rounded-xl bg-red-600 hover:bg-red-700 text-white font-black text-xs uppercase tracking-wider transition-all flex items-center justify-center gap-1.5 shadow-md cursor-pointer mt-2"
                        >
