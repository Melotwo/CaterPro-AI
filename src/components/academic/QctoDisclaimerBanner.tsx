...
export const QctoDisclaimerBanner: React.FC<{ compact?: boolean }> = ({ compact = false }) => {
  return (
    <div 
      ...
      className="bg-gradient-to-r from-red-900/30 via-slate-900 to-red-950/10 border-l-4 border-red-600 rounded-3xl p-5 sm:p-6 shadow-xl relative overflow-hidden text-left border border-white/10"
    >
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 relative z-10">
        <div className="flex items-start gap-3.5">
          <div className="w-10 h-10 rounded-2xl bg-red-950/40 text-red-400 border border-red-800/60 flex items-center justify-center shrink-0 mt-0.5 shadow-inner">
            <ShieldCheck className="w-5 h-5" />
          </div>

          <div className="space-y-1.5">
            <div className="flex flex-wrap items-center gap-2">
              <span className="px-2.5 py-0.5 rounded-md bg-red-950/40 text-red-400 border border-red-800/60 text-[10px] font-black uppercase tracking-wider">
            ...
            <p className="text-xs text-red-100/90 leading-relaxed max-w-4xl font-medium">
              <strong className="text-red-400 font-bold">Institutional Advisory: </strong> 
        ...
        {!compact && (
          <div className="shrink-0 flex items-center gap-3 pt-2 md:pt-0 border-t md:border-t-0 border-white/10">
            <div className="text-right hidden lg:block">
              ...
              <div className="text-xs font-mono font-bold text-red-400">SAQA 101697 • 102296 • 110644</div>
            </div>
            <div className="w-9 h-9 rounded-xl bg-slate-800/80 border border-red-600/50 flex items-center justify-center text-red-400 text-base">
              🇿🇦
            </div>
          </div>
        )}
